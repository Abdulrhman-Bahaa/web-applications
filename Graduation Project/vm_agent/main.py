import subprocess, time, os, glob, json, sys

def cleanup():
    for f in ["snapshot_before.json", "snapshot_after.json", "etw_report.json", "Sha256_dynamic.json"]:
        try:
...
def take_snapshot(name):
    try:
        subprocess.run([sys.executable, "snapshot.py"], check=True, capture_output=True)
...
def run_analysis(malware_path, duration=12):
    ...
    print("\n[*] STEP 1: BEFORE snapshot...")
    if not take_snapshot("snapshot_before.json"): return False
    
    with open("snapshot_before.json") as f:
        before = json.load(f)
    print(f"    Processes: {len(before.get('processes', []))}")
    
    # Step 2: Run malware with ETW
    print(f"\n[*] STEP 2: Running malware with ETW ({duration}s)...")
    ...
    try:
        ...
        # Start ETW monitor (NO timeout - let it run full duration)
        etw_proc = subprocess.Popen(
            [sys.executable, "etw_monitor.py", str(duration), str(malware_proc.pid)]
        )
    ...
    print("\n[*] STEP 3: AFTER snapshot...")
    time.sleep(1)
    if not take_snapshot("snapshot_after.json"): return False
